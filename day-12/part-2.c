...
bool
small_cave_visited_twice(struct cave *caves, size_t len) {
	for (size_t i = 0; i < len; i++) {
		if (strcmp(caves[i].name, "start") == 0)
			continue;
		if (strcmp(caves[i].name, "end") == 0)
			continue;
		if (islower(caves[i].name[0]) && caves[i].visited == 2)
			return true;
	}
...
explore(char const *from, struct cave *caves, size_t len) {
	...
	for (size_t i = 0; i < current->len; i++) {
		...
		if (strcmp(next->name, "start") == 0)
			continue;
		if (islower(next->name[0]))
			if (next->visited && small_cave_visited_twice(caves, len))
				continue;
